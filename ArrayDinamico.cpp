...
//    size = 5     maxSize = 10
	if(size == maxSize){
		cout << endl << "Estendo l'array..." << endl;
		maxSize += BUF_SIZE +1;
...
void ArrayDinamico::remove(int idx){ // elimina ptr[idx]
...
double ArrayDinamico::operator[](int idx) const{ // a[i] sola lettura
...
double & ArrayDinamico::operator[](int idx){ // a[i] = x
		if(idx <0 || idx>= size){
			throw out_of_range("ArrayDinamico::operator[] -> indice fuori dal range");
		}
		return ptr[idx];
